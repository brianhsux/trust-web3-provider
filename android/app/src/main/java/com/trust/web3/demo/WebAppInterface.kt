...
class WebAppInterface(
    ...
    fun postMessage(json: String) {
        Log.d("chromium", "privateKey: $privateKey,addr: $addr, pubKey: $pubKey");
        Log.d("chromium", "json: $json");
        val obj = JSONObject(json)
        println(obj)
        val id = obj.getLong("id")
        val method = DAppMethod.fromValue(obj.getString("name"))
        val network = obj.getString("network")
        Log.d("chromium", "DAppMethod.REQUESTACCOUNTS, method: $method");
        Log.d("chromium", "DAppMethod.REQUESTACCOUNTS, network: $network");

        when (method) {
            DAppMethod.REQUESTACCOUNTS -> {
                Log.d("chromium", "DAppMethod.REQUESTACCOUNTS");
                    ...
                    okButton {
                        ...
                        webView.post {
                            ...
                            webView.evaluateJavascript(callback) { value ->
                                Log.d("chromium", "DAppMethod.REQUESTACCOUNTS: $value");
                                println(value)
                            }
                        }
                    }
            ...
            DAppMethod.SIGNMESSAGE -> {
                Log.d("chromium", "DAppMethod.SIGNMESSAGE");
